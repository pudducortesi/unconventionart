...
function createFallback(value,name='') {
  ...
  const width={slim:.89,regular:1,broad:1.13}[a.build],shoulders=width*a.shoulders/100,hips=width*a.hips/100,waist=width*a.waist/100;
  body.scale.y=a.height/100;
  const material=(color,roughness=.8)=>new T.MeshStandardMaterial({color,roughness});
  const mats={skin:material(a.skin),hair:material(a.hair,.7),outfit:material(a.outfit),trousers:material(a.trousers),eyes:material('#f4f0e8',.3),iris:material(a.eyes,.3),pupil:material('#131313',.2),shoe:material(a.shoes,.6),sole:material('#b7b1a8'),lip:material(new T.Color(a.skin).multiplyScalar(.62)),trim:material(new T.Color(a.outfit).multiplyScalar(.65))};
  const sphere=new T.SphereGeometry(1,a.model==='studio'?24:16,a.model==='studio'?16:12),geometries=new Set([sphere]);
  ...
  const points=[[0,.86],[.14,.87],[.16,.94],[.155,1.03],[.19,1.20],[.215,1.27],[.17,1.31],[.075,1.34],[0,1.34]].map(([r,y])=>new T.Vector2(r*(y<.93?hips:y<1.08?waist:shoulders),y));
  mesh(body,new T.LatheGeometry(points,20),'outfit',0,0,0,1,1,.64);
  oval(body,'trousers',0,.87,0,.17*hips,.115,.105);
  taper(body,'trim',.025,.154*waist,.157*waist,0,.94,0,.66);
  taper(body,'skin',.14,.055,.065,0,1.35,0,.9);
  const head=joint(body,'head',0,1.50,0);head.scale.x=a.faceWidth/100;
  oval(head,'skin',0,.015,0,.133,.185,.125);
  oval(head,'skin',0,-.073,-.017,.103*a.jaw/100,.105,.108).name='avatar-jaw';
  oval(head,'skin',0,-.005,-.126,.023*a.nose/100,.040*a.nose/100,.029*a.nose/100).name='avatar-nose'; // nose
  oval(head,'lip',0,-.075,-.111,.039*a.lipSize/100,.009*a.lipSize/100,.011).name='avatar-lips';
  ...
  for(const side of [-1,1]){
    ...
    const eye=joint(head,'eye',side*.050,.041,-.111);lids.push(eye);eye.scale.setScalar(a.eyeSize/100);
  ...
  if(a.style==='bald'){}else if(a.style==='mohawk'){
    for(let i=0;i<7;i++)oval(head,'hair',0,.19-Math.abs(i-3)*.012,-.095+i*.033,.025,.05,.027);
  }else if(a.style==='shaved'){
    ...
    if(a.style==='bun')oval(head,'hair',0,.13,.13,.073,.073,.07);
  ...
  if(a.beard==='stubble'||a.beard==='full'){
    const beard=mesh(head,new T.SphereGeometry(1,24,12,0,Math.PI*2,Math.PI*.52,Math.PI*.46),'hair',0,-.025,-.012,.106*a.jaw/100,a.beard==='full'?.17:.115,.113);beard.name='avatar-beard';
  }
  if(a.beard==='moustache'||a.beard==='full')for(const side of [-1,1])oval(head,'hair',side*.023,-.055,-.12,.027,.009,.012);
  ...
  for(const side of [-1,1]){
    const arm=joint(body,side<0?'leftUpperArm':'rightUpperArm',side*.208*shoulders,1.265,0);arm.rotation.z=side*.075;arms.push(arm);
    oval(arm,'outfit',side*.012,-.047,0,.069,.09,.070);
    taper(arm,a.garment==='dress'?'skin':'outfit',.205,.060,.046,side*.015,-.145,0);
    const elbow=joint(arm,'elbow',side*.015,-.255,0);elbows.push(elbow);
    oval(elbow,'outfit',0,0,0,.047,.048,.047);
    taper(elbow,a.garment==='tshirt'||a.garment==='dress'?'skin':'outfit',.19,.044,.033,0,-.10,0);
    taper(elbow,a.garment==='tshirt'||a.garment==='dress'?'skin':'trim',.028,.034,.034,0,-.198,0);
    oval(elbow,'skin',0,-.245,-.007,.036,.055,.023);
    oval(elbow,'skin',-side*.028,-.231,-.011,.015,.030,.014);
    const leg=joint(body,side<0?'leftUpperLeg':'rightUpperLeg',side*.089*hips,.87,0);legs.push(leg);
  ...
  root.userData.animate=(dt,time,speed=0,waving=false)=>{
    ...
    lids.forEach(e=>e.scale.y=open*a.eyeSize/100);
