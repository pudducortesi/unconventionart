...
export function createPublicGallery({url,serviceKey,publicKey,fetchImpl=fetch}) {
  ...
  return async request => {
    if (request.method === 'OPTIONS') return new Response(null,{status:204,headers:{...headers,'Access-Control-Allow-Methods':'GET, OPTIONS','Access-Control-Allow-Headers':'apikey, range'}});
    ...
    try {
      if (path === 'catalogue') {
        ...
        const videos=videoRows.map(row=>({id:row.id,title:row.title,hallIndex:row.hall_index,url:`${url}/functions/v1/gallery-public/video/${row.id}${publicKey?'?apikey='+encodeURIComponent(publicKey):''}`}));
        return json({works,videos,collections:[{id:'atelier',title:'UnconventionArt',description:'La collezione fotografica.',color:'#b99363'}],hero:works[0]?.image || null,revision:[...rows,...videoRows].map(r=>`${r.id}:${r.updated_at}`).join('|')});
      }
      const parts = path.split('/');
      if(parts.length===2 && parts[0]==='video' && idPattern.test(parts[1])) {
        const id=parts[1], response=await fetchImpl(`${url}/rest/v1/gallery_videos?id=eq.${id}&published=eq.true&select=id,video_path`,{headers:apiHeaders,signal:AbortSignal.timeout(10000)});
        if(!response.ok)throw Error();const rows=await response.json();
        if(rows.length!==1||rows[0].id!==id||![`${id}/clip.mp4`,`${id}/clip.webm`].includes(rows[0].video_path))return json({error:'Not found'},404);
        const range=request.headers.get('range');if(range&&!/^bytes=\d*-\d*$/.test(range))return json({error:'Invalid range'},416);
        const media=await fetchImpl(`${url}/storage/v1/object/authenticated/gallery-videos/${rows[0].video_path}`,{headers:{...apiHeaders,...(range?{Range:range}:{})},signal:AbortSignal.timeout(30000)});
        if(media.status===416)return new Response(null,{status:416,headers:{...headers,'Content-Range':media.headers.get('content-range')||'bytes */0'}});
        if(!media.ok)throw Error();
        const out={...headers,'Content-Type':rows[0].video_path.endsWith('.mp4')?'video/mp4':'video/webm','Accept-Ranges':'bytes','Access-Control-Expose-Headers':'Content-Range, Content-Length, Accept-Ranges'};
        for(const name of ['content-range','content-length'])if(media.headers.has(name))out[name]=media.headers.get(name);
