...
- Orientamento tra partecipanti: sala, piano terra/scala/soppalco e distanza approssimativa in linea d’aria. **Raggiungi** avvia un percorso verso uno spazio vicino all’ultima posizione ricevuta, usando porte e scale; i comandi manuali lo interrompono. Il comando richiede una presenza dell’incontro aggiornata negli ultimi cinque secondi e rispetta le sale aperte. Riaprire Incontri durante una visita mostra subito i presenti.
- Messaggi non letti sul pulsante Incontri e nella relativa scheda, con comando per leggere i nuovi messaggi. La cronologia iniziale e i propri messaggi non aumentano il contatore; vengono contati solo i messaggi ancora disponibili nella finestra restituita dal server. La chat mantiene la posizione di lettura e aggiorna i messaggi senza ricreare l’intera cronologia. Gli invii duplicati sono bloccati, le bozze modificate durante l’invio sono conservate e gli errori permettono un nuovo invio esplicito. Uscire dall’incontro cancella la bozza locale.
...
## Verifica orientamento negli incontri (settembre 2026)

`npm run check`: build e 169 test. I nuovi test eseguono i percorsi con il solver di movimento, comprese scale e passaggi tra soppalchi; verificano distanza di arresto, sale chiuse, posizioni irraggiungibili, pareti e scadenza delle presenze. I dati provengono dal roster già autorizzato dell’incontro, senza nuove tabelle o chiamate di localizzazione. Il percorso è verso una posizione ricevuta, non segue continuamente il partecipante. La prova visiva con due dispositivi resta da completare.
