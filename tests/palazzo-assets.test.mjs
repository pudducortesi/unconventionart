...
test('the historical ceiling photograph deploy as bounded WebP textures, outside the artwork catalogue', async () => {
  const catalogue = await readFile('data/catalogue.json', 'utf8');
  let totalBytes = 0;
  for (const name of ['colonna-ceiling']) {
    ...
    assert(metadata.width >= 1024 && metadata.height >= 1024, 'Keep painted details legible');
  ...
  assert(totalBytes < 800_000, 'Keep the historical ceiling lightweight');
});
