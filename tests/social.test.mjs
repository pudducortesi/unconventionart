...
test('Social RPC enforces authentication, room isolation, identity, blocks, rate limits and moderation',async t=>{
  ...
  await clearLimit(ids[0],'save_profile');
  const glasses=await rpc('save_profile',{name:'Visitor 0',avatar:{...DEFAULT_AVATAR,glasses:'round',frame:'gold'}});
  assert.equal(glasses.avatar.glasses,'round');assert.equal(glasses.avatar.frame,'gold');
  await clearLimit(ids[0],'save_profile');
  await assert.rejects(rpc('save_profile',{name:'Visitor 0',avatar:{...DEFAULT_AVATAR,glasses:'https://evil.test/model.glb'}}),/ua_invalid_avatar/);
  await clearLimit(ids[0],'save_profile');
  ...
  const legacy={...DEFAULT_AVATAR};delete legacy.model;delete legacy.glasses;delete legacy.frame;
