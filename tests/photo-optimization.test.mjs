...
test('photo pipeline preserves source and catalogue, creates smaller bounded derivatives with quality checks',async t=>{
 ...
 assert.equal(result.catalogue.works[0].image,before.works[0].image);
...
test('production assets contain optimized paths, original HD, and no research or compressor code',async()=>{
 ...
 for(const [i,work] of published.works.entries()){
  assert.equal(work.image,source.works[i].image);
  for(const path of [work.thumbnail,work.mobilePreview,work.preview])await stat(join('dist',path));
  assert.deepEqual(await readFile(join('dist',work.image)),await readFile(work.image));
 }
